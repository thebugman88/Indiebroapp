...
## Active follow-up: Artist Assistant account workspace

Owner: Artist Assistant frontend. Keep the existing authenticated API contract. Replace the unowned, separately written browser keys with one versioned UID-owned local snapshot; bind every load/save/export/reset to the mounted auth session. Legacy localStorage/IndexedDB stays untouched and is never implicitly claimed. Guest work is transient. On corruption or storage failure, report the problem rather than overwrite old data or claim a successful save. This is browser persistence, not cloud sync or device-level encryption. Test account switches, stale callbacks, quota errors, reset/export ownership and secret stripping. Rollback must retain new and legacy data and must not restore cross-account reads.
