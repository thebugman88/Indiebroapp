...
## Re-audit follow-up — privacy, browser isolation and account recovery (2026-08-30)

Baseline: `4f6978d`. This batch addresses the three findings left open above; these are code fixes, not live rollout certification.

- Judgment Zone uses an explicit per-viewer response allowlist on list, submission and review responses. Only uploaders see their ownership signature and owner UID. A judge sees artist/title/lyrics only after a recorded review; other reviewers' UIDs are never returned. A forged saved-vault list does not unlock identity. Review validation still uses the complete private record. Audio now streams through a verified-account endpoint using an opaque track ID instead of exposing owner IDs in bucket paths. Playback fetches with authentication, rejects stale downloads after account changes, revokes temporary blob URLs, and reports unavailable audio instead of substituting synthesized music. Existing shared signed URLs expire on their original schedule.
- Blind judging cannot prevent recognition from the audio, embedded audio metadata, or voluntary feedback. The user-facing disclosure now states those limits; audio transcoding/metadata stripping is not implemented. Do not claim absolute anonymity.
- RoyaltyOps stores each account in a separate IndexedDB database. Each mounted workspace holds its original account guard so late OCR results, saves, reads and resets cannot retarget another account. Export actions check the current session. The root artist catalog uses one versioned, owner-tagged snapshot per account; malformed/foreign data and write failures are reported. Both views remount on account changes. Old shared catalog keys and the old RoyaltyOps database are preserved but never automatically imported. Explicit legacy recovery remains separate work. Browser separation is not encryption or cross-device backup.
- Sentinel rate limiting returns 429 and Retry-After without quarantining users. Signature-based quarantine expires after five minutes. Restart restoration respects the latest block/unblock entry and the original expiry (including legacy timestamps). Verified-account cancellation and verified-admin remediation have a separate ten-per-minute limit and remain reachable during ordinary quarantine. Authentication, admin permissions and billing ownership checks still apply.

Regression checks cover privacy before/after reviews, signature and storage-path leakage, actual IndexedDB CRUD via fake-indexeddb, account switching, reset scope, delayed database opens, corrupt catalogs, rate-window expiry, cancellation access, and unblock persistence through a fresh Node process. Root typecheck, 36 security tests, root build, production smoke and both affected standalone app build/secret checks pass. The API-level privacy regression also verifies that forged viewer parameters cannot unlock identities; the playback regression verifies that logout invalidates pending downloads. Firebase integration tests still require Java 21+ (workspace has Java 17); real two-account browser playback/storage tests, private bucket IAM, provider calls and Stripe/Firebase staging remain launch gates.
